(function () {
  ...
  function startGame() {
    if (loadedCount !== assetKeys.length) {
      overlayText.textContent = "加载张先生中";
      return;
    }
  ...
  function endGame() {
    ...
    updateScore();
    overlayText.textContent = "按空格 / ↑ / 点击再跑一把。";
  ...
  function playDeathSound() {
    var oldBgmVol = bgm.volume;
    bgm.volume = 0.05;  // 压低 BGM 让死亡音效清晰
    var picked = deathSounds[Math.floor(Math.random() * deathSounds.length)];
    var audio = new Audio(picked);
    audio.volume = 1.0;
